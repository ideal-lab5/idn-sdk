...
fn can_fail_write_pulse_when_beacon_config_not_set() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Drand::try_submit_asig(RuntimeOrigin::none(), asig.try_into().unwrap(), start, end, signature.into()),
...
use sp_runtime::{MultiSignature, transaction_validity::{InvalidTransaction, TransactionPriority, TransactionSource}};
...
fn validate_unsigned_has_correct_properties() {
	...
	new_test_ext().execute_with(|| {
		...
		let alice_keypair = sp_core::sr25519::Pair::from_string("//Alice", None).unwrap();
		let signature =MultiSignature::Sr25519(alice_keypair.sign(&encoded_data));
		let call = Call::try_submit_asig { asig: formatted.clone(), start: 1000, end: 1001, signature: signature };
...
fn validate_unsigned_provides_unique_tags() {
	...
	new_test_ext().execute_with(|| {
		...
		let signature =MultiSignature::Sr25519(alice_keypair.sign(&encoded_data));
		let call1 =
			Call::try_submit_asig { asig: asig1.try_into().unwrap(), start: 1000, end: 1001, signature: signature.clone() };

		let call2 =
			Call::try_submit_asig { asig: asig2.try_into().unwrap(), start: 1002, end: 1003, signature: signature };
