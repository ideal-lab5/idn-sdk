...
use crate::traits::PulseConsumer;
use bp_idn::{
	types::{
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type Consumer: PulseConsumer<Pulse, SubscriptionId, DispatchResultWithPostInfo>;
	...
	pub enum Event<T: Config> {
		...
		RandomnessConsumed {
			round: <Pulse as PulseTrait>::Round,
			sub_id: SubscriptionId,
		},
		SubFeesQuoteConsumed {
			quote: SubFeesQuote,
		},
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn consume_pulse(
			...
			T::Consumer::consume_pulse(pulse, sub_id)?;
		...
		pub fn consume_sub_fees_quote(
			...
			// let fee = Self::calculate_subscription_fee(credits, frequency)?;

			// Self::deposit_event(Event::SubFeesQuoteConsumed { round: 0, sub_id: [0u8; 32] });
...
impl<T: Config> Pallet<T> {
	...
	/// [`pallet::consume_sub_fees_quote`] function along with the `quote_id`.
	/// The `quote_id` is generated by this function and used to identify the quote when returned.
	pub fn request_sub_fees_quote(
		credits: Credits,
		frequency: IdnBlockNumber,
	) -> Result<u128, Error<T>> {
