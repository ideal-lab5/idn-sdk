...
//! `minimal_chain_spec.json` for minimal and `parachain_chain_spec.json` for parachaine2e
//! templates.
//!
//! To start all tests here we should run:
//! `cargo test -p idn-sdk-e2e-tests --features e2e`
...
mod smoke {
	...
	async fn minimal_template_block_production_test() -> Result<(), anyhow::Error> {
		...
		let config = get_config(NetworkSpec {
			relaychain_cmd: "idn-sdk-kitchensink-node",
			..Default::default()
		})?;
	...
	// #[tokio::test(flavor = "multi_thread")]
	// async fn omni_node_with_minimal_runtime_block_production_test() ->
	// Result<(), anyhow::Error> {
	// 	let _ = env_logger::try_init_from_env(
	// 		env_logger::Env::default().filter_or(env_logger::DEFAULT_FILTER_ENV, "info"),
	// 	);
	...
	// #[tokio::test(flavor = "multi_thread")]
	// async fn omni_node_with_parachain_runtime_block_production_test() ->
	// Result<(), anyhow::Error> {
	// 	let _ = env_logger::try_init_from_env(
	// 		env_logger::Env::default().filter_or(env_logger::DEFAULT_FILTER_ENV, "info"),
	// 	);
