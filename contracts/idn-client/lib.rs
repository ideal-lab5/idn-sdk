...
impl IdnClientImpl {
	...
	fn construct_create_subscription_xcm(
		...
		let encoded_params =
			codec::Encode::encode(&(credits, frequency, call_index, metadata, pulse_filter));

		// Create the XCM program to call the create_subscription function
		let call_create_subscription = [idn_manager_pallet_index, idn_manager_create_sub_index]
			.into_iter()
	...
	fn construct_pause_subscription_xcm(&self, subscription_id: SubscriptionId) -> Xcm<()> {
		...
		let call_pause_subscription = [idn_manager_pallet_index, idn_manager_pause_sub_index]
			.into_iter()
			.chain(encoded_params)
			.collect::<Vec<_>>();
	...
	fn construct_reactivate_subscription_xcm(&self, subscription_id: SubscriptionId) -> Xcm<()> {
		...
		let call_reactivate_subscription =
			[idn_manager_pallet_index, idn_manager_reactivate_sub_index]
				.into_iter()
				.chain(encoded_params)
				.collect::<Vec<_>>();
	...
	fn construct_update_subscription_xcm(
		...
		let pulse_filter = pulse_filter.unwrap_or_default();
		let encoded_params =
			codec::Encode::encode(&(subscription_id, credits, frequency, pulse_filter));

		// Create the XCM program to call the update_subscription function
		let call_update_subscription = [idn_manager_pallet_index, idn_manager_update_sub_index]
			.into_iter()
			.chain(encoded_params)
			.collect::<Vec<_>>();
	...
	fn construct_kill_subscription_xcm(&self, subscription_id: SubscriptionId) -> Xcm<()> {
		...
		let call_kill_subscription = [idn_manager_pallet_index, idn_manager_kill_sub_index]
			.into_iter()
			.chain(encoded_params)
			.collect::<Vec<_>>();
...
mod tests {
	...
	fn test_message_content_validation() {
		...
		let frequency = 5;
		let create_message =
			client.construct_create_subscription_xcm(credits, call_index, frequency, None, None);
	...
	fn test_client_encoding_decoding() {
		...
		let message = decoded.construct_create_subscription_xcm(10, [0, 1], 5, None, None);
