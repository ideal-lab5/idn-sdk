...
mod example_consumer {
	use idn_contracts::xcm::{
		...
		Error, IdnClient, IdnConsumer, ContractCallParams
	};
	use bp_idn::types::RequestReference;
	...
	impl ExampleConsumer {
		...
		pub fn request_sub_info(
			...
			self.idn_client
				.request_sub_info(subscription_id, req_ref, metadata, None)
				.map_err(ContractError::IdnClientError)?;
			Ok(())
		}

		#[ink(message)]
		pub fn did_request_sub_info_work(&self) -> bool {
